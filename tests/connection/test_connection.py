...
class TestConnection(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tcp_echo(self):
        ...
        start = time.time()
        conn = await connect("tcpbin.com", 4242, timeout=10.0)
        end = time.time()
        ...
        start = time.time()
        async with connect("tcpbin.com", 4242, timeout=10.0) as conn:
            end = time.time()
            self.assertIsInstance(conn, Connection)
            self.log(logging.INFO, f"connected in {end - start} seconds")
    ...
    async def test_tcp_latency(self):
        ...
        delay = await tcp_latency_test("tcpbin.com", 4242, timeout=10.0)
